...
package com.biiway.stockassistant.components.alert
{
	...
	public class AlertForm extends UIComponent implements IFontContextComponent
	{
		...
		override protected function createChildren():void
		{
			...
			var iconClass:Class = Alert(Object(parentDocument).parent).iconClass;
			if (iconClass && !icon)
			{
				icon = new iconClass();
				addChild(icon);
			...
			var alert:Alert = Alert(Object(parentDocument).parent);
		...
		override protected function commitProperties():void
		{
			...
			if (defaultButtonChanged && defaultButton)
			{
				...
				Alert(Object(parentDocument).parent).defaultButton = defaultButton;
				
				if (parent is IFocusManagerContainer)
				{
					var sm:ISystemManager = Alert(Object(parentDocument).parent).systemManager;
		...
		override protected function measure():void
		{
			...
			var title:String = Alert(Object(parentDocument).parent).title;
			var lineMetrics:TextLineMetrics =Alert(Object(parentDocument).parent).measureText(title);
//				Alert(Object(parentDocument).parent).getTitleTextField().
...
//			var textMetrics:TextLineMetrics = label.measureText(text); 
		...
		mx_internal function createTextField(childIndex:int):void
		{
			if (!textField)
			{
				...
				textField.text = Alert(Object(parentDocument).parent).text;
				textField.multiline = true;
				textField.wordWrap = true;
		 ...
		 *  @private
		 */
		private function createButton(label:String, name:String):mx.controls.Button
		{
		...
		{
			var buttonFlags:uint = Alert(Object(parentDocument).parent).buttonFlags;
